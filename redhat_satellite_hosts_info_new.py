...
# The following fields are in https://host/api/v2/hosts
            ...
            for host in results:
                ...
                service_level = host.get("subscription_facet_attributes", {}).get("service_level")

                host_data.append((name, os_name, rh_version, ip, domain, org, location, hostgroup, content_view, lifecycle_env, registered_at, service_level ))

            # Write to hosts.csv
            with open("hosts.csv", "w", newline="") as csvfile:
                csv_writer = csv.writer(csvfile)
                csv_writer.writerow(["Name", "Operating System", "Version", "IP Address", "Domain Name", "Organization", "Location", "HostGroup Name", "Content View", "LifeCycle Environment", "Registered Date", "Service Level" ])
